...
<title>${message("admin.supplier.shipList")} - Powered By openteach inc.</title>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.supplier.shipList")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="ship_list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as orderItem]
				<tr>
					...
					<td>
						[#if orderItem.platformPaymentStatus == "unpaid" && orderItem.shippingStatus == "shipped" && orderItem.order.orderStatus == "completed"]
						<a href="bill.jhtml?orderItemIds=${orderItem.id}&supplierId=${orderItem.supplier.id}">[${message("admin.supplier.checkout")}]</a>
						[/#if]
					</td>
				</tr>
			[/#list]
