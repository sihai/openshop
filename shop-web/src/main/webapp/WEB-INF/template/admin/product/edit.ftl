...
<title>${message("admin.product.edit")} - Powered By openteach inc.</title>
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post" enctype="multipart/form-data">
		...
		<ul id="tab" class="tab">
			<li>
				<input type="button" value="${message("admin.product.supplier")}" />
			</li>
			<li>
		...
		<table class="input tabContent">
			<tr>
				<th>
					<span class="requiredField">*</span>${message("Product.supplier")}:
				</th>
				<td>
					<select name="supplierId">
						<option value="">${message("admin.common.choose")}</option>
						[#list suppliers as supplier]
							<option value="${supplier.id}" [#if product.supplier?? && supplier.id == product.supplier.id] selected="selected"[/#if]>
								${supplier.name}
							</option>
						[/#list]
					</select>
			...
			<tr>
				<th>
					<span class="requiredField">*</span>${message("Product.rebatePoint")}:
				</th>
				<td>
					<input type="text" name="rebatePoint" class="text" [#if product.rebatePoint??] value="${product.rebatePoint * 100}" [/#if] maxlength="3" title="${message("admin.product.rebatePointTitle")}"/> %
				</td>
			</tr>
		</table>
			...
			[#list product.productCategory.parameterGroups as parameterGroup]
				...
				[#list parameterGroup.parameters as parameter]
					<tr>
						<th>${parameter.name}:</th>
						<td>
							<input type="text" name="parameter_${parameter.id}" class="text" value="${product.getParameterValue(parameter)}" maxlength="200" />
						</td>
					</tr>
				[/#list]
			[/#list]
