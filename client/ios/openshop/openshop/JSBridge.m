...
#import "JSBridge.h"
    ...
    #define JSBRIDGE_WEAK __weak
#else
    #define JSBRIDGE_WEAK __unsafe_unretained
#endif

typedef NSDictionary JSBridgeMessage;

@implementation JSBridge {
    
    JSBRIDGE_WEAK JSBRIDGE_WEBVIEW_TYPE* _webView;
    
    JSBRIDGE_WEAK id _webViewDelegate;
    ...
    JSBridgeHandler _messageHandler;
    
    NSBundle *_resourceBundle;
    
#if defined JSBRIDGE_PLATFORM_IOS
...
+ (instancetype)bridgeForWebView:(JSBRIDGE_WEBVIEW_TYPE*)webView webViewDelegate:(JSBRIDGE_WEBVIEW_DELEGATE_TYPE*)webViewDelegate handler:(JSBridgeHandler)messageHandler {
    return [self bridgeForWebView:webView webViewDelegate:webViewDelegate handler:messageHandler resourceBundle:nil];
}

+ (instancetype)bridgeForWebView:(JSBRIDGE_WEBVIEW_TYPE*)webView webViewDelegate:(JSBRIDGE_WEBVIEW_DELEGATE_TYPE*)webViewDelegate handler:(JSBridgeHandler)messageHandler resourceBundle:(NSBundle*)bundle
{
...
- (void)send:(id)data responseCallback:(JSBridgeResponseCallback)responseCallback {
...
- (void)registerHandler:(NSString *)handlerName handler:(JSBridgeHandler)handler {
    [_messageHandlers setObject: handler forKey: handlerName];
}
...
- (void)_sendData:(id)data responseCallback:(JSBridgeResponseCallback)responseCallback handlerName:(NSString*)handlerName {
...
- (void)_queueMessage:(JSBridgeMessage*)message {
...
- (void)_dispatchMessage:(JSBridgeMessage*)message {
    ...
    if ([[NSThread currentThread] isMainThread]) {
        ...
        __strong JSBRIDGE_WEBVIEW_TYPE* strongWebView = _webView;
...
- (void)_flushMessageQueue {
    NSString * messageQueueString = [_webView stringByEvaluatingJavaScriptFromString:@"jsbridge._fetchQueue();"];
    
    id messages = [self _deserializeMessageJSON: messageQueueString];
    if (![messages isKindOfClass:[NSArray class]]) {
        NSLog(@"jsbridge: WARNING: Invalid %@ received: %@", [messages class], messages);
        return;
    }
    for (JSBridgeMessage* message in messages) {
        if (![message isKindOfClass:[JSBridgeMessage class]]) {
        ...
        if (responseId) {
            ...
            if (callbackId) {
                responseCallback = ^(id responseData) {
                    ...
                    JSBridgeMessage* msg = @{ @"responseId":callbackId, @"responseData":responseData };
            ...
            if (message[@"handlerName"]) {
                ...
                if (!handler) {
                    NSLog(@"JSBRIDGE Warning: No handler for %@", message[@"handlerName"]);
 ...
 **********************************/
#if defined JSBRIDGE_PLATFORM_OSX

- (void) _platformSpecificSetup:(JSBRIDGE_WEBVIEW_TYPE*)webView webViewDelegate:(JSBRIDGE_WEBVIEW_DELEGATE_TYPE*)webViewDelegate handler:(JSBridgeHandler)messageHandler resourceBundle:(NSBundle*)bundle{
...
- (void)webView:(WebView *)webView didFinishLoadForFrame:(WebFrame *)frame
{
    ...
    if (![[webView stringByEvaluatingJavaScriptFromString:@"typeof jsbridge == 'object'"] isEqualToString:@"true"]) {
 ...
 **********************************/
#elif defined JSBRIDGE_PLATFORM_IOS

- (void) _platformSpecificSetup:(JSBRIDGE_WEBVIEW_TYPE*)webView webViewDelegate:(id<UIWebViewDelegate>)webViewDelegate handler:(JSBridgeHandler)messageHandler resourceBundle:(NSBundle*)bundle {
    _messageHandler = messageHandler;
    _webView = webView;
    _webViewDelegate = webViewDelegate;
    _messageHandlers = [NSMutableDictionary dictionary];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (_numRequestsLoading == 0 && ![[webView stringByEvaluatingJavaScriptFromString:@"typeof jsbridge == 'object'"] isEqualToString:@"true"]) {
        NSBundle *bundle = _resourceBundle ? _resourceBundle : [NSBundle mainBundle];
        NSString *filePath = [bundle pathForResource:@"jsbridge" ofType:@"js"];
        NSString *js = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];
        NSLog(@"stringByEvaluatingJavaScriptFromString:%@", [webView stringByEvaluatingJavaScriptFromString:js]);
    }
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSURL *url = [request URL];
    __strong typeof(_webViewDelegate) strongDelegate = _webViewDelegate;
    if ([[url scheme] isEqualToString: JSBRIDGE_SCHEMA]) {
        if ([[url host] isEqualToString: JSBRIDGE_QUEUE_MESSAGE]) {
            [self _flushMessageQueue];
        } else {
            NSLog(@"JSBridge: WARNING: Received unknown jsbridge command %@://%@", JSBRIDGE_SCHEMA, [url path]);
        }
        return NO;
