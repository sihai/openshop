...
<title>${message("admin.order.edit")} - Powered By openteach inc.</title>
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input tabContent">
			...
			<tr>
				<th>
					${message("Order.rebate")}:
				</th>
				<td>
					${currency(order.rebate, true)}
				</td>
				<th>
					${message("Order.supplierIncome")}:
				</th>
				<td>
					${currency(order.amount.subtract(order.rebate), true)}
				</td>
			</tr>
			<tr>
				<th>
		...
		<table id="orderItemTable" class="input tabContent">
			...
			<tr class="title">
				...
				<th>
					${message("admin.order.productRebatePoint")}
				</th>
				<th>
			...
			[#list order.orderItems as orderItem]
				<tr class="orderItemTr">
					...
					<td>
						[#if !orderItem.isGift]
							<input type="text" name="orderItems[${orderItem_index}].rebatePoint" class="text orderItemPrice" value="${orderItem.rebatePoint}" maxlength="3" style="width: 60px;" />
							<input type="hidden" name="orderItems[${orderItem_index}].rebate" value="0" />
						[#else]
							<input type="hidden" name="orderItems[${orderItem_index}].rebatePoint" value="0" />
							<input type="hidden" name="orderItems[${orderItem_index}].rebate" value="0" />
							-
						[/#if]
					</td>
