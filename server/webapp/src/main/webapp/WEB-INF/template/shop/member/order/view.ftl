...
<!--<![endif]-->
	<head>
		...
		<title>${message("shop.member.order.view")}[#if systemShowPowered] - Powered By openteach inc.[/#if]</title>
				...
				<div class="input order">
					...
					<div class="top">
						...
						<span class="handle">
							...
							[/#if]
							[#if !order.expired && order.orderStatus == "confirmed" && order.paymentStatus == "paid" && order.shippingStatus == "shipped"]
								<a href="javascript:;" id="received" class="button">${message("shop.member.order.received")}</a>
							[/#if]
						</span>
						<div class="tips">
							[#if order.expired]
	...
	<script type="text/javascript">
		$().ready(function() {
			...
			$received.click(function() {
				if (confirm("${message("shop.member.order.receivedConfirm")}")) {
					$.ajax({
						url: "received.jhtml?sn=${order.sn}",
						type: "POST",
						dataType: "json",
						cache: false,
						success: function(message) {
