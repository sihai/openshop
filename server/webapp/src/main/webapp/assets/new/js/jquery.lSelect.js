		...
		lSelect: function(options) {
			...
			return this.each(function() {
				...
				function addSelect($position, parentId, currentId) {
					...
					var select = '<select name="' + selectName + '" class="text">';
					...
					select += '</select>';
					return $(select).css(settings.cssStyle).insertAfter($position.parent().children(":last" )).bind("change", function() {
						var $this = $(this);
						if ($this.val() == "") {
					...
					}).selectBoxIt({ hideEffectSpeed:200, showEffectSpeed:200, showEffect:"fadeIn", hideEffect:"fadeOut"});
				}
			});
