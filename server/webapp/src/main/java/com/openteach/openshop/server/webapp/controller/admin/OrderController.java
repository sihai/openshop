		...
		for (OrderItem orderItem : order.getOrderItems()) {
			if (orderItem.getId() != null) {
				...
				if (pOrderItem.getIsGift()) {
					...
					orderItem.setRebate(orderItem.getPrice().multiply(new BigDecimal(orderItem.getQuantity()).multiply(orderItem.getRebatePoint())));
				}
			} else {
				Product product = productService.findBySn(orderItem.getSn());
				if (product == null) {
			...
			totalRebate = totalRebate.add(orderItem.getRebate());
		}
