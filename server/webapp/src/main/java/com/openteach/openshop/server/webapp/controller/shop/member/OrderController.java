...
import com.openteach.openshop.server.biz.service.ReceiverService;
...
public class OrderController extends BaseController {
	...
	public String payment(String sn, ModelMap model) {
		...
		model.addAttribute("order", order);
		return "/shop/member/order/newPayment";
	}
	...
	public @ResponseBody
	Message received(String sn) {
		Order order = orderService.findBySn(sn);
		if (order != null && memberService.getCurrent().equals(order.getMember()) && !order.isExpired() && order.getOrderStatus() == OrderStatus.confirmed && order.getPaymentStatus() == PaymentStatus.paid && ShippingStatus.shipped == order.getShippingStatus()) {
			if (order.isLocked(null)) {
				return Message.warn("shop.member.order.locked");
			}
			orderService.complete(order, null);
			
			logger.error("try to confirm payment received");
			for(Payment payment : order.getPayments()) {
				PaymentPlugin paymentPlugin = pluginService.getPaymentPlugin(payment.getPaymentPluginId());
				paymentPlugin.confirmReceived(order, payment);
			}
			return SUCCESS_MESSAGE;
		}
		return ERROR_MESSAGE;
