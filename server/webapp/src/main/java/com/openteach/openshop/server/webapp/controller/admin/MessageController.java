...
public class MessageController extends BaseController {
	...
	public String send(Long draftMessageId, String username, String title, String content, @RequestParam(defaultValue = "false") Boolean isDraft, HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		if (isDraft) {
			addFlashMessage(redirectAttributes, com.openteach.openshop.server.biz.Message.success("admin.message.saveDraftSuccess"));
			return "redirect:draft.jhtml";
		} else {
			addFlashMessage(redirectAttributes, com.openteach.openshop.server.biz.Message.success("admin.message.sendSuccess"));
	...
	public String reply(Long id, String content, HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		if ((forMessage.getReceiver() == null && !forMessage.getSenderDelete()) || (forMessage.getSender() == null && !forMessage.getReceiverDelete())) {
			...
			addFlashMessage(redirectAttributes, com.openteach.openshop.server.biz.Message.success("admin.message.replySuccess"));
			return "redirect:list.jhtml";
		}
	}
	...
	public @ResponseBody
	com.openteach.openshop.server.biz.Message delete(Long[] ids) {
		if (ids != null) {
			for (Long id : ids) {
