...
import com.openteach.openshop.server.service.Setting.ReviewAuthority;
import com.openteach.openshop.server.service.Setting.RoundType;
import com.openteach.openshop.server.service.Setting.StockAllocationTime;
import com.openteach.openshop.server.service.Setting.WatermarkPosition;
		...
		try {
			...
			if (setting.getIsDevelopmentEnabled()) {
				if (!templateUpdateDelay.equals("0") || !messageCacheSeconds.equals("0")) {
					...
					properties.store(outputStream, "aigechibaole PROPERTIES");
				}
			} else {
				if (templateUpdateDelay.equals("0") || messageCacheSeconds.equals("0")) {
					outputStream = new FileOutputStream(resource.getFile());
