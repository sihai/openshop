...
public class ProductServiceImpl extends BaseServiceImpl<Product, Long> implements ProductService, DisposableBean {
	...
	private long favourTime = System.currentTimeMillis();
	...
	@Transactional(readOnly = true)
	public Long count(Supplier supplier, Member favoriteMember, Boolean isMarketable, Boolean isList, Boolean isTop, Boolean isGift, Boolean isOutOfStock, Boolean isStockAlert) {
		return productDao.count(supplier, favoriteMember, isMarketable, isList, isTop, isGift, isOutOfStock, isStockAlert);
	}
	...
	@Override
	public long viewFavours(Long id) {
		Ehcache cache = cacheManager.getEhcache(Product.FAVORS_CACHE_NAME);
		Element element = cache.get(id);
		Long favours;
		if (element != null) {
			favours = (Long) element.getObjectValue();
		} else {
			Product product = productDao.find(id);
			if (product == null) {
				return 0L;
			}
			favours = (long) product.getFavoriteMembers().size();
			cache.put(new Element(id, favours));
		}
	...
	@Override
	public void updateProductFavorCache(Long id) {
		Ehcache cache = cacheManager.getEhcache(Product.FAVORS_CACHE_NAME);
		Element element = cache.get(id);
		Long favours;
		if (element != null) {
