...
public class OrderDAOImpl extends BaseDAOImpl<Order, Long> implements OrderDAO {
	...
	public List<Order> findList(Member member, Integer count, List<Filter> filters, List<com.openteach.openshop.server.service.Order> orders) {
		if (member == null) {
	...
	public void autoConfirmReceived() {
		String jpql = "select orders from Order orders where orders.orderStatus = 1 and orders.paymentStatus = 2 and orders.shippingStatus <= 2 AND orders.shipedDate > :time";
		List<Order> orders = entityManager.createQuery(jpql, Order.class).setParameter("time", DateUtils.addDays(new Date(), -7)).getResultList();
		if (orders != null) {
			for (Order order : orders) {
				if (order != null) {
